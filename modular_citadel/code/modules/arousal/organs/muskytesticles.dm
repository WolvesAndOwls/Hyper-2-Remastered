/obj/item/organ/genital/testicles/musk/
	name 					= "musky testicles"
	desc 					= "A male reproductive organ. It smells very potent."
	...
	var/gas					= /datum/gas/pheromone/
	var/active = TRUE
	var/power_draw = 0
	var/power_draw_static = 0
	var/power_draw_dynamic_mol_coeff = 1	//DO NOT USE DYNAMIC SETTINGS UNTIL SOMEONE MAKES A USER INTERFACE/CONTROLLER FOR THIS!
	var/power_draw_dynamic_kpa_coeff = 0.1
	var/spawn_id = null
	var/spawn_temp = T20C
...
/obj/item/organ/genital/testicles/musk/on_life()
	if(QDELETED(src))
		return
	if(!reagents || !owner)
		return
...
/obj/item/organ/genital/testicles/musk/generate_musk()
	spawn_id = /datum/gas/pheromone/
	generate_cum()


/obj/item/organ/genital/testicles/musk/proc/generate_cum()
...
/obj/item/organ/genital/testicles/musk/update_link()
...
/obj/item/organ/genital/testicles/musk/proc/send_full_message(msg = "Your balls finally feel full, again.")
	if(owner && istext(msg))
		to_chat(owner, msg)
		return TRUE

/obj/item/organ/genital/testicles/musk/update_appearance()
	switch(size)
		if(0.1 to 3)
			size_name = "average"
		//if(1.1 to 2)
