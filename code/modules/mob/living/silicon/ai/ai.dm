...
// Can set AI eye to any turf that exists and is onstation
/mob/living/silicon/ai/proc/setEyeLoc(turf/T)
...
/mob/living/silicon/ai/proc/botcall()
	...
	for (Bot in GLOB.alive_mob_list)
		if((Bot.z == ai_Zlevel || (is_station_level(Bot.z) && is_station_level(ai_Zlevel))) && !Bot.remote_disabled) //Only non-emagged bots on the same Z-level or onstation with the AI are detected!
...
/mob/living/silicon/ai/canUseTopic(atom/movable/M, be_close=FALSE, no_dextery=FALSE, no_tk=FALSE)
	if(control_disabled || incapacitated())
		...
		return FALSE
	// Always let the AI use their PDA if they're not disabled or incapacitated
	if(M == builtInPDA)
		return TRUE
